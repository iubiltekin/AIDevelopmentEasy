...
import { Save, RefreshCw, Server, Wifi, WifiOff, Brain, Zap, RotateCcw, Settings as SettingsIcon } from 'lucide-react';
import { settingsApi, type LLMSettingsDto, type LLMUsageStatsDto } from '../services/api';
import { PageLayout, ErrorAlert } from '../components';
...
export function Settings() {
  ...
  const [llmStats, setLlmStats] = useState<LLMUsageStatsDto | null>(null);
  ...
  const loadLLMStats = async () => {
    try {
      const data = await settingsApi.getLlmStats();
      setLlmStats(data ?? null);
    } catch {
      // ignore
    }
  };
  ...
  const saveLLMSettings = async () => {
    try {
      await settingsApi.updateLlm(llmSettings);
      setLlmSaved(true);
      setLlmError(null);
  ...
  const resetLLMStats = async () => {
    try {
      await settingsApi.resetLlmStats();
      loadLLMStats();
    } catch {
      // ignore
    }
  ...
  return (
    <PageLayout
      title="Settings"
      description="Configure the application and LLM settings"
      titleIcon={SettingsIcon}
      onRefresh={() => { loadLLMSettings(); loadLLMStats(); checkHealth(); }}
    >
      <div className="space-y-6 max-w-4xl">
        {llmError && <ErrorAlert message={llmError} />}
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-6">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Max Input Tokens (Prompt Limit)</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Max Output Tokens (Completion Limit)</label>
              <div className="flex items-center gap-4">
                <input
                  type="range"
                  min="500"
              ...
              <div className="mt-1 text-xs text-slate-500">≈ {tokensToKB(llmSettings.maxCompletionTokens)} KB</div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Cost per 1K Input Tokens ($)</label>
                <input
                  type="number"
                  step="0.001"
                  min="0"
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-2">Cost per 1K Output Tokens ($)</label>
                <input
                  type="number"
                  step="0.001"
                  min="0"
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className={`relative w-12 h-6 rounded-full transition-colors ${llmSettings.showPromptInfo ? 'bg-purple-600' : 'bg-slate-600'}`}
              >
                <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${llmSettings.showPromptInfo ? 'translate-x-6' : 'translate-x-0.5'}`} />
              </button>
            </div>
        ...
        {llmStats && (
          <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-slate-900/50 rounded-lg p-4">
                <div className="text-sm font-mono text-white">In: {llmStats.totalPromptTokens.toLocaleString()}</div>
                <div className="text-sm font-mono text-white">Out: {llmStats.totalCompletionTokens.toLocaleString()}</div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-slate-900 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-500"
              />
            </div>
            <div className="flex items-center justify-between">
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-xl p-6">
          ...
          <div className="space-y-3 text-slate-400">
            <p>
              <strong className="text-white">AI Development Easy</strong> is a multi-agent framework for AI-assisted software development.
            </p>
            <p>
              It uses multiple specialized AI agents (Planner, Coder, Debugger, Reviewer) to process stories and generate high-quality code.
            </p>
            <div className="pt-4 border-t border-slate-700">
              <p className="text-sm">Version: <span className="text-white">1.0.0</span></p>
              <p className="text-sm">Made with ❤️ using Azure OpenAI</p>
            </div>
          </div>
        </div>
